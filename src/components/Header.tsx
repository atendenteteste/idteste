      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between h-[85%] md:h-auto">
        ...
        <div className="hidden md:flex md:items-center">
                ...
                >
                  {isLoading ? (
                    ...
                    localeProducts.map((product, index) => (
                      <div key={index} className="py-1 border-l-4 border-transparent hover:border-l-4 hover:border-purple-600 transition-all">
                        <Link 
          ...
          <Link 
            ...
            className="ml-6 px-5 py-2.5 rounded-full text-white text-sm font-medium hover:opacity-90 transition-colors"
                  ...
                  {isLoading ? (
                    ...
                    localeProducts.map((product, index) => (
                      <Link 
                        key={index}
                        href={`/${currentLocale}/${product.slug}`}
                        className="block py-2 px-4 text-sm text-gray-700 hover:text-purple-600"
                        onClick={() => setMobileMenuOpen(false)}
                      >
                        {product.title}
                      </Link>
            ...
            {!isLoading && pages.length > 0 && (
              <div className="hidden">
                ...
                <div className="space-y-2">
                  <Link 
                    href="/pt-br"
                    className={`block py-2 px-4 text-sm ${currentLocale === 'pt-br' ? 'text-purple-600 font-medium' : 'text-gray-700'}`}
                    onClick={() => setMobileMenuOpen(false)}
                  >
                    Português (Brasil)
                  </Link>
                  {pages.map((page) => (
                    <Link 
                      key={page.id}
                      href={`/${page.slug}`}
                      className={`block py-2 px-4 text-sm ${currentLocale === page.slug ? 'text-purple-600 font-medium' : 'text-gray-700'}`}
                      onClick={() => setMobileMenuOpen(false)}
                    >
                      {page.title}
                    </Link>
